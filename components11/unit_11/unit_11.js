...
function f8() {
    ...
    let newArray = new Array(d8.length + 1);

    newArray[0] = inputValue;
...
function f11() {
    ...
    document.querySelector('.out-11').textContent = index;
}
...
function f12() {
    ...
    for (let i = 0; i < d12.length; i++) {
        if (d12[i] === number && foundIndex === -1) {
            foundIndex = i;
        }
    }

    document.querySelector('.out-12').textContent = foundIndex;
}
...
function f13() {
    ...
    for (let i = 0; i < d13.length; i++) {
        reversedArray[i] = d13[d13.length - 1 - i];
    }
    
    showArr('.out-13', reversedArray);
}
...
function f17() {
    ...
    let totalLength = d171.length + d172.length;
    
    for (let i = 0; i < totalLength; i++) {
        if (i < d171.length) {
            d17.push(d171[i]);
        } else {
            d17.push(d172[i - d171.length]);
        }
    }
...
function f18() {
    ...
    let isIncluded = d18.includes(inputValue);
    
    document.querySelector('.out-18').textContent = isIncluded;
}
